...
def parse_args():
    parser = argparse.ArgumentParser(description="Visualize PCA of Bottleneck Features")
    ...
    parser.add_argument("--output", type=str, default="pca_bottleneck.png", help="Output image path")
...
def main():
    ...
    plt.scatter(pca_results[idx_src_fresh, 0], pca_results[idx_src_fresh, 1], 
                c='#377eb8', marker='o', alpha=0.7, label=f'Source Fresh ({sum(idx_src_fresh)})')
                
    # Source Rotten
    idx_src_rotten = (domains == 0) & (all_labels == 1)
    plt.scatter(pca_results[idx_src_rotten, 0], pca_results[idx_src_rotten, 1], 
                c='#ff7f00', marker='o', alpha=0.7, label=f'Source Rotten ({sum(idx_src_rotten)})')
                
    # Target Fresh
    idx_tgt_fresh = (domains == 1) & (all_labels == 0)
    plt.scatter(pca_results[idx_tgt_fresh, 0], pca_results[idx_tgt_fresh, 1], 
                c='#4daf4a', marker='^', alpha=0.7, label=f'Target Fresh ({sum(idx_tgt_fresh)})')
                
    # Target Rotten
    idx_tgt_rotten = (domains == 1) & (all_labels == 1)
    plt.scatter(pca_results[idx_tgt_rotten, 0], pca_results[idx_tgt_rotten, 1], 
                c='#e41a1c', marker='^', alpha=0.7, label=f'Target Rotten ({sum(idx_tgt_rotten)})')
                
    plt.title(f"PCA on Bottleneck Features ({all_feats.shape[1]}-dim)")

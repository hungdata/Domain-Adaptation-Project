...
"""
CDAN-E + MCC (Conditional Domain Adversarial Network with Entropy and MCC)
=====================================
Loss = L_CE_source + L_domain(CDAN-E) + λ * β * L_MCC_target
...
def minimum_class_confusion_loss(logits, temperature: float = 2.5, eps: float = 1e-8):
    ...
    ent = ent.detach()                                        # MCC paper (Jin et al., ECCV 2020)
    weights = 1.0 + torch.exp(-ent)                           # [B]
    weights = (weights / (weights.sum() + eps)) * batch_size  # [B]
    weighted_probs = probs * weights.unsqueeze(1)             # [B, C]
    confusion = torch.matmul(weighted_probs.t(), probs)       # [C, C]
    confusion = confusion / (confusion.sum(dim=1, keepdim=True) + eps)
    off_diag = confusion.sum() - torch.diagonal(confusion).sum()
    loss = off_diag / num_classes
    return loss

def parse_args():
    parser = argparse.ArgumentParser(description="CDANE")
    ...
    parser.add_argument("--mcc-weight", type=float, default=1.0)
    parser.add_argument("--mcc-temp", type=float, default=2.5)
...
def main():
    ...
    grl         = GRL()
    modules     = {"F_ext": F_ext, "bottleneck": bottleneck, "classifier": classifier, "domain_disc": domain_disc}
    ...
    for epoch in range(start_epoch, args.epochs + 1):
        ...
        F_ext.train(); bottleneck.train(); classifier.train(); domain_disc.train()
        tgt_iter = InfiniteIterator(tgt_train_loader)

        epoch_cls_loss = 0.0; epoch_domain_loss = 0.0; epoch_mcc_loss = 0.0; epoch_total_loss = 0.0; epoch_entropy_weight = 0.0
        correct = 0; total = 0
        ...
        for src_imgs, src_labels in tqdm(src_train_loader, desc=f'Epoch [{epoch:03d}/{args.epochs}]', leave=False, dynamic_ncols=True):
            ...
            if lambda_p <= 0.0:
                ...
                mcc_loss    = torch.zeros((), device=device)
                total_loss  = cls_loss
            else:
                # DA PHASE - GHÉP BATCH
                ...
                tgt_features = cdan_multilinear_map(tgt_bn, tgt_probs)

                concat_for_disc = torch.cat([src_features, tgt_features], dim=0)
                concat_d = domain_disc(grl(concat_for_disc, alpha=lambda_p))
                
                src_d, tgt_d = concat_d[:bs_src], concat_d[bs_src:]

                # Tính weight entropy cho mỗi mẫu
                src_weight, src_raw_mean = get_cdane_weight(src_probs)
                tgt_weight, tgt_raw_mean = get_cdane_weight(tgt_probs)
                epoch_entropy_weight += (src_raw_mean + tgt_raw_mean) * src_labels.size(0)

                d_loss_src = F.binary_cross_entropy_with_logits(src_d, torch.ones_like(src_d), weight=src_weight.view(-1, 1))
                d_loss_tgt = F.binary_cross_entropy_with_logits(tgt_d, torch.zeros_like(tgt_d), weight=tgt_weight.view(-1, 1))
                domain_loss = 0.5 * (d_loss_src + d_loss_tgt)
                
                mcc_loss = minimum_class_confusion_loss(tgt_logits, temperature=args.mcc_temp)

                total_loss = cls_loss + domain_loss + lambda_p * args.mcc_weight * mcc_loss
            ...
            epoch_cls_loss    += cls_loss.item() * src_labels.size(0)
            epoch_domain_loss += domain_loss.item() * src_labels.size(0)
            epoch_mcc_loss    += mcc_loss.item() * src_labels.size(0)
            epoch_total_loss  += total_loss.item() * src_labels.size(0)
            correct += (src_logits.argmax(1) == src_labels).sum().item()
            total   += bs

        epoch_cls_loss    /= max(total, 1)
        epoch_domain_loss /= max(total, 1)
        epoch_mcc_loss    /= max(total, 1)
        epoch_total_loss  /= max(total, 1)
        epoch_entropy_weight /= (2 * max(len(src_train_loader), 1))
        ...
        ckpt_state = build_base_ckpt(METHOD, epoch, modules, optimizer, args, 0.0, args.lr, current_lr, args.epochs, args.warmup, extra={"lambda_adv": lambda_p, "mcc_weight": args.mcc_weight})
        if epoch % 10 == 0:
        ...
        losses = {"total": epoch_total_loss, "cls": epoch_cls_loss, "domain": epoch_domain_loss, "mcc": epoch_mcc_loss}
        ...
        append_csv(epoch_log_path, {
            ...
            "source_cls_loss": f"{epoch_cls_loss:.6f}", "domain_loss": f"{epoch_domain_loss:.6f}", "lmmd_loss": "0", "mcc_loss": f"{epoch_mcc_loss:.6f}", "entropy_weight_mean": f"{epoch_entropy_weight:.6f}",

...
"""
DSANE (DSAN with Entropy-weighted LMMD)
=====================================
Loss tổng: L = L_CE_source + λ · L_LMMD_E_target

Các quy tắc áp dụng:
1. TÁCH BATCH (Độc lập Ảnh): Source và Target đi qua mạng hoàn toàn độc lập với nhau để tách biệt thống kê BatchNorm.
...
from core.uda_utils import (
    ...
    FeatureExtractor, Bottleneck, ClassifierHead, compute_lmmd, entropy_of,
...
def parse_args():
    parser = argparse.ArgumentParser(description="DSANE_MCC_SEP")
...
def main():
    ...
    print("=" * 60)
    print(f"[{METHOD}] DSANE (ENTROPY-WEIGHTED) + MCC - TÁCH BATCH + WARMUP LOCK")
    print("=" * 60)
    ...
    for epoch in range(start_epoch, args.epochs + 1):
        ...
        for src_imgs, src_labels in tqdm(src_train_loader, desc=f'Epoch [{epoch:03d}/{args.epochs}]', leave=False, dynamic_ncols=True):
            ...
            if lambda_p <= 0.0:
                ...
                # Luồng Target
                tgt_feat   = F_ext(tgt_imgs)
                tgt_bn     = bottleneck(tgt_feat)
                tgt_logits = classifier(tgt_bn)

                cls_loss   = ce_loss_fn(src_logits, src_labels)

                # DSAN-E: LMMD with Entropy Weighting
                # đã fixed dsane thành 1+e**(-H)
                tgt_probs = F.softmax(tgt_logits, dim=1)
                ent = entropy_of(tgt_probs)
                reliability = 1.0 + torch.exp(-ent)
                tgt_softmax = (tgt_probs * reliability.unsqueeze(1)).detach()
                
                lmmd_loss = compute_lmmd(src_bn, tgt_bn, src_labels, tgt_softmax)

                mcc_loss = minimum_class_confusion_loss(tgt_logits, temperature=args.mcc_temp)
                total_loss = cls_loss + lambda_p * lmmd_loss + lambda_p * args.mcc_weight * mcc_loss
            ...
            epoch_mcc_loss    += mcc_loss.item() * bs if isinstance(mcc_loss, torch.Tensor) else 0
        ...
        losses = {"total": epoch_total_loss, "cls": epoch_cls_loss, "domain": 0.0, "lmmd": epoch_lmmd_loss, "mcc": epoch_mcc_loss}
        ...
        append_csv(epoch_log_path, {
            ...
            "source_cls_loss": f"{epoch_cls_loss:.6f}", "domain_loss": "0", "lmmd_loss": f"{epoch_lmmd_loss:.6f}",
            "mcc_loss": f"{epoch_mcc_loss:.6f}", "entropy_weight_mean": "NA",

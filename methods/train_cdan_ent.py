...
=====================================
Loss tổng: L = L_CE_source + L_CDAN (với GRL tự động nhân λ)
...
def main():
    ...
    print("=" * 60)
    print(f"[{METHOD}] CDAN - GHÉP BATCH + WARMUP LOCK")
    print("=" * 60)
    ...
    for epoch in range(start_epoch, args.epochs + 1):
        ...
        epoch_cls_loss = 0.0; epoch_domain_loss = 0.0; epoch_total_loss = 0.0; epoch_ent_loss = 0.0
        correct = 0; total = 0
        ...
        for src_imgs, src_labels in tqdm(src_train_loader, desc=f'Epoch [{epoch:03d}/{args.epochs}]', leave=False, dynamic_ncols=True):
            ...
            if lambda_p <= 0.0:
                ...
                d_loss_src = bce_loss_fn(src_d, torch.ones_like(src_d))
                d_loss_tgt = bce_loss_fn(tgt_d, torch.zeros_like(tgt_d))
            ...
            epoch_domain_loss += domain_loss.item() * bs
        ...
        append_csv(epoch_log_path, {
            ...
            "source_cls_loss": f"{epoch_cls_loss:.6f}", "domain_loss": f"{epoch_domain_loss:.6f}", "lmmd_loss": "0", "mcc_loss": "0", "entropy_weight_mean": f"{args.ent_weight}",

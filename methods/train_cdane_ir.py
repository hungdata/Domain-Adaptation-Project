...
def main():
    ...
    if os.path.exists(last_model_path):
        ...
        print(f"[*] Resumed from epoch {ckpt['epoch']}, best acc: {ckpt.get('best_src_val_acc', 0.0):.4f}")
    else:
    ...
    for epoch in range(start_epoch, args.epochs + 1):
        ...
        F_ext.train(); bottleneck.train(); classifier.train(); aux_disc.train(); domain_disc.train()
        tgt_iter = InfiniteIterator(tgt_train_loader)

        epoch_cls_loss = 0.0; epoch_aux_domain_loss = 0.0; epoch_ir_weight = 0.0
        ...
        for src_imgs, src_labels in tqdm(src_train_loader, desc=f'Epoch [{epoch:03d}/{args.epochs}]', leave=False, dynamic_ncols=True):
            ...
            if lambda_p <= 0.0:
                ...
                total_loss = cls_loss + domain_loss + aux_domain_loss
        ...
        losses = {"total": epoch_total_loss, "cls": epoch_cls_loss, "domain": epoch_domain_loss, "aux_dom": epoch_aux_domain_loss}

...
"""
DSANC (DSAN with Consistency-weighted LMMD)
=====================================
Loss tổng: L = L_CE_source + λ · L_LMMD_C_target

Các quy tắc áp dụng:
1. GHÉP BATCH (Concat Forward): Source và Target nối lại rồi đi qua mạng 1 lần duy nhất.
...
from core.uda_utils import (
    ...
    FeatureExtractor, Bottleneck, ClassifierHead, DomainDiscriminator, compute_lmmd, entropy_of, GRL,
...
def parse_args():
    parser = argparse.ArgumentParser(description="DSANC")
...
def main():
    ...
    modules     = {"F_ext": F_ext, "bottleneck": bottleneck, "classifier": classifier, "aug_disc": aug_disc}

    param_groups = build_param_groups(F_ext, bottleneck, classifier, lr=args.lr)
    # Add aug_disc parameters to the optimizer (with 1.0 lr multiplier)
    ...
    print("=" * 60)
    print(f"[{METHOD}] DSANC (ADV-AUGMENTATION) - TWO-VIEW + GRL")
    print("=" * 60)
    ...
    for epoch in range(start_epoch, args.epochs + 1):
        ...
        F_ext.train(); bottleneck.train(); classifier.train(); aug_disc.train()
        ...
        for src_imgs, src_labels in tqdm(src_train_loader, desc=f'Epoch [{epoch:03d}/{args.epochs}]', leave=False, dynamic_ncols=True):
            ...
            if lambda_p <= 0.0:
                ...
                # DSAN-C: LMMD on Weak features + Adversarial Augmentation Consistency
                tgt_probs_w = F.softmax(tgt_w_logits, dim=1).detach()
                tgt_probs_s = F.softmax(tgt_s_logits, dim=1).detach()
                
                # Consistency score for LMMD weighting
                consistency = F.cosine_similarity(tgt_probs_w, tgt_probs_s, dim=1)
                tgt_softmax = (tgt_probs_w * consistency.unsqueeze(1)).detach()
                lmmd_loss = compute_lmmd(src_bn, tgt_w_bn, src_labels, tgt_softmax)

                # Adversarial Augmentation Invariance (ADA)
                ...
                aug_features_grl = grl_layer(aug_features, alpha=lambda_p)
                ...
                cons_loss = F.binary_cross_entropy_with_logits(aug_disc_logits, aug_labels)

                # Total Loss: CE + lambda * (LMMD + Aug_Adv_Loss)
                total_loss = cls_loss + lambda_p * (lmmd_loss + cons_loss)
            ...
            epoch_cons_loss   += cons_loss.item() * bs
        ...
        losses = {"total": epoch_total_loss, "cls": epoch_cls_loss, "domain": epoch_cons_loss, "lmmd": epoch_lmmd_loss}
        ...
        append_csv(epoch_log_path, {
            ...
            "lambda_adv": "0", "lambda_lmmd": f"{lambda_p:.6f}", "warmup_status": "WARMUP" if lambda_p <= 0 else "DA_ACTIVE",
            "train_total_loss": f"{epoch_total_loss:.6f}",
            "source_cls_loss": f"{epoch_cls_loss:.6f}", "domain_loss": f"{epoch_cons_loss:.6f}", "lmmd_loss": f"{epoch_lmmd_loss:.6f}",
            "mcc_loss": "0", "entropy_weight_mean": "NA",

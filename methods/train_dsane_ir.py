...
from core.uda_utils import (
    ...
    images_from_batch,
    set_rng_state,
    set_seed, get_device, BOTTLENECK_DIM, get_gpu_memory_mb,
    get_source_train_loader, get_source_val_loader, get_source_test_loader,
    get_target_train_loader, get_target_test_loader,
    FeatureExtractor, Bottleneck, ClassifierHead, DomainDiscriminator, compute_lmmd, entropy_of,
...
def main():
    ...
    for epoch in range(start_epoch, args.epochs + 1):
        ...
        F_ext.train(); bottleneck.train(); classifier.train(); aux_disc.train()
        tgt_iter = InfiniteIterator(tgt_train_loader)

        epoch_cls_loss = 0.0; epoch_aux_domain_loss = 0.0; epoch_ir_weight = 0.0; epoch_lmmd_loss = 0.0; epoch_total_loss = 0.0
        correct = 0; total = 0
        ...
        for src_imgs, src_labels in tqdm(src_train_loader, desc=f'Epoch [{epoch:03d}/{args.epochs}]', leave=False, dynamic_ncols=True):
            ...
            if lambda_p <= 0.0:
                ...
                total_loss = cls_loss + lambda_p * lmmd_loss + aux_domain_loss
        ...
        losses = {"total": epoch_total_loss, "cls": epoch_cls_loss, "domain": epoch_aux_domain_loss, "lmmd": epoch_lmmd_loss}
        ...
        append_csv(epoch_log_path, {
            ...
            "source_cls_loss": f"{epoch_cls_loss:.6f}", "domain_loss": f"{epoch_aux_domain_loss:.6f}", "lmmd_loss": f"{epoch_lmmd_loss:.6f}",
            "mcc_loss": "0", "entropy_weight_mean": f"{epoch_ir_weight:.4f}",

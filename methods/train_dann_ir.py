...
=====================================
Loss tổng: L = L_CE_source + L_domain (với GRL tự động nhân λ vào gradient)
...
def main():
    ...
    print("=" * 60)
    print(f"[{METHOD}] DANN - GHÉP BATCH + WARMUP LOCK")
    print("=" * 60)
    ...
    for epoch in range(start_epoch, args.epochs + 1):
        ...
        for src_imgs, src_labels in tqdm(src_train_loader, desc=f'Epoch [{epoch:03d}/{args.epochs}]', leave=False, dynamic_ncols=True):
            ...
            if lambda_p <= 0.0:
                ...
                # Khắc phục lỗi BatchNorm: Gộp chung Source và Target trước khi đưa vào Domain Discriminator
                # để chúng dùng chung một bộ mean/var của BatchNorm
                concat_for_disc = torch.cat([src_bn, tgt_bn], dim=0)
                ...
                # GRL tự động xử lý lambda nên không cần nhân lambda vào domain_loss

...
"""
CDANE (Conditional Domain Adversarial Network with Entropy Conditioning)
=====================================
Loss tổng: L = L_CE_source + L_CDAN_E (với GRL tự động nhân λ và Entropy Weight)
...
from core.uda_utils import (
    ...
    FeatureExtractor, Bottleneck, ClassifierHead, DomainDiscriminator, GRL,
    cdan_multilinear_map, entropy_of,
    ...
    SEED, NUM_CLASSES, EPOCHS, WARMUP_EPOCHS, LR, BATCH_SIZE,
    BACKBONE_LR_FACTOR, BOTTLENECK_DIM,
)

METHOD = "CDANC"

CDANC_EPOCH_LOG_FIELDS = [
    "epoch", "method", "seed", "batch_size", "learning_rate",
    "lambda_adv", "lambda_lmmd", "warmup_status",
    "train_total_loss", "source_cls_loss", "domain_loss", "aug_loss", "lmmd_loss", "mcc_loss",
    "entropy_weight_mean",
    "source_train_acc",
...
class TargetTwoViewDataset(Dataset):
...
def parse_args():
    parser = argparse.ArgumentParser(description="CDANC")
...
def get_cdane_weight(probs):
    ent = entropy_of(probs).detach()
    raw_weight = 1.0 + torch.exp(-ent)
    weight = (probs.size(0) * raw_weight / torch.sum(raw_weight)).unsqueeze(1)
    return weight, raw_weight.mean().item()
...
def main():
    ...
    tgt_train_ds = TargetTwoViewDataset(os.path.join(args.data_root, "target", "train_unlabeled", "images"))
    tgt_train_loader = DataLoader(tgt_train_ds, batch_size=args.batch_size, shuffle=True, drop_last=True, num_workers=args.num_workers)
    ...
    domain_disc = DomainDiscriminator(in_dim=BOTTLENECK_DIM * NUM_CLASSES).to(device)
    aug_disc    = DomainDiscriminator(in_dim=BOTTLENECK_DIM).to(device)
    grl         = GRL()
    modules     = {"F_ext": F_ext, "bottleneck": bottleneck, "classifier": classifier, "domain_disc": domain_disc, "aug_disc": aug_disc}
    ...
    param_groups.append({"params": aug_disc.parameters(), "lr": args.lr})
    ...
    if os.path.exists(last_model_path):
        ...
        init_csv(epoch_log_path, CDANC_EPOCH_LOG_FIELDS)
        init_csv(target_eval_path, TARGET_EVAL_FIELDS)
        init_csv(source_eval_path, SOURCE_EVAL_FIELDS)
        
    print("=" * 60)
    print(f"[{METHOD}] CDANE (Entropy) - GHÉP BATCH + WARMUP LOCK")
    print("=" * 60)
    ...
    for epoch in range(start_epoch, args.epochs + 1):
        ...
        F_ext.train(); bottleneck.train(); classifier.train(); domain_disc.train(); aug_disc.train()
        tgt_iter = InfiniteIterator(tgt_train_loader)

        epoch_cls_loss = 0.0
        epoch_domain_loss = 0.0
        epoch_aug_loss = 0.0
        epoch_entropy_weight = 0.0
        ...
        for src_imgs, src_labels in tqdm(src_train_loader, desc=f'Epoch [{epoch:03d}/{args.epochs}]', leave=False, dynamic_ncols=True):
            ...
            if lambda_p <= 0.0:
                ...
                aug_loss    = torch.zeros((), device=device)
                total_loss  = cls_loss
                
                bs_tgt = 0
            else:
                # DA PHASE - TWO-VIEW CONCATENATION
                tgt_batch = next(tgt_iter)
                tgt_imgs_w = tgt_batch[0].to(device)
                tgt_imgs_s = tgt_batch[1].to(device)

                # Nối ảnh (Source + Target Weak + Target Strong)
                concat_imgs = torch.cat((src_imgs, tgt_imgs_w, tgt_imgs_s), dim=0)
                ...
                # Cắt kết quả
                bs_tgt = tgt_imgs_w.size(0)
                
                src_bn       = concat_bn[:bs_src]
                tgt_w_bn     = concat_bn[bs_src:bs_src+bs_tgt]
                tgt_s_bn     = concat_bn[bs_src+bs_tgt:]
                
                src_logits   = concat_logits[:bs_src]
                tgt_w_logits = concat_logits[bs_src:bs_src+bs_tgt]
                tgt_s_logits = concat_logits[bs_src+bs_tgt:]

                cls_loss   = ce_loss_fn(src_logits, src_labels)

                # ==========================================
                # 1. CDANE: Domain Adversarial (Source vs Target Weak)
                # ==========================================
                src_probs = F.softmax(src_logits, dim=1).detach()
                tgt_w_probs = F.softmax(tgt_w_logits, dim=1).detach()
                
                src_features = cdan_multilinear_map(src_bn, src_probs)
                tgt_w_features = cdan_multilinear_map(tgt_w_bn, tgt_w_probs)

                concat_for_domain_disc = torch.cat([src_features, tgt_w_features], dim=0)
                concat_domain_d = domain_disc(grl(concat_for_domain_disc, alpha=lambda_p))
                
                src_d, tgt_w_d = concat_domain_d[:bs_src], concat_domain_d[bs_src:]

                # Entropy Conditioning Weight
                src_weight, src_raw_mean = get_cdane_weight(src_probs)
                tgt_weight, tgt_raw_mean = get_cdane_weight(tgt_w_probs)
                epoch_entropy_weight += (src_raw_mean + tgt_raw_mean) * bs_src

                d_loss_src = F.binary_cross_entropy_with_logits(src_d, torch.ones_like(src_d), weight=src_weight.view(-1, 1))
                d_loss_tgt = F.binary_cross_entropy_with_logits(tgt_w_d, torch.zeros_like(tgt_w_d), weight=tgt_weight.view(-1, 1))
                domain_loss = 0.5 * (d_loss_src + d_loss_tgt)

                # ==========================================
                # 2. ADA: Augmentation Adversarial (Target Weak vs Target Strong)
                # ==========================================
                # We want Feature Extractor to fool aug_disc into thinking strong aug is weak aug
                aug_features = torch.cat([tgt_w_bn, tgt_s_bn], dim=0)
                aug_features_grl = grl(aug_features, alpha=lambda_p)
                aug_disc_logits = aug_disc(aug_features_grl)
                
                # Labels: 0 for Weak (Real), 1 for Strong (Augmented)
                aug_labels = torch.cat([
                    torch.zeros(bs_tgt, 1, device=device),
                    torch.ones(bs_tgt, 1, device=device)
                ], dim=0)
                
                aug_loss = F.binary_cross_entropy_with_logits(aug_disc_logits, aug_labels)

                # Total Loss
                total_loss = cls_loss + domain_loss + aug_loss

            optimizer.zero_grad()
            total_loss.backward()
            optimizer.step()

            epoch_cls_loss    += cls_loss.item() * bs_src
            epoch_domain_loss += domain_loss.item() * bs_src
            epoch_aug_loss    += aug_loss.item() * bs_tgt
            epoch_total_loss  += total_loss.item() * bs_src
            correct += (src_logits.argmax(1) == src_labels).sum().item()
            total   += bs_src

        epoch_cls_loss    /= max(total, 1)
        epoch_domain_loss /= max(total, 1)
        epoch_aug_loss    /= max(total, 1)
        epoch_entropy_weight /= (2 * max(total, 1))
        ...
        ckpt_state = build_base_ckpt(METHOD, epoch, modules, optimizer, args, 0.0, args.lr, current_lr, args.epochs, args.warmup, extra={"lambda_adv": lambda_p})
        if epoch % 10 == 0:
        ...
        losses = {"total": epoch_total_loss, "cls": epoch_cls_loss, "domain": epoch_domain_loss, "aug": epoch_aug_loss, "lmmd": 0.0}
        ...
        append_csv(epoch_log_path, {
            ...
            "lambda_adv": f"{lambda_p:.6f}", "lambda_lmmd": "0", "train_total_loss": f"{epoch_total_loss:.6f}",
            "source_cls_loss": f"{epoch_cls_loss:.6f}", "domain_loss": f"{epoch_domain_loss:.6f}", "aug_loss": f"{epoch_aug_loss:.6f}", "lmmd_loss": "0", "mcc_loss": "0", "entropy_weight_mean": f"{epoch_entropy_weight:.6f}",
            "warmup_status": "WARMUP" if lambda_p <= 0 else "DA_ACTIVE",
            ...
            "target_monitor_fresh_acc": f"{tgt_mon['fresh_acc']:.6f}", "target_monitor_rotten_acc": f"{tgt_mon['rotten_acc']:.6f}",
